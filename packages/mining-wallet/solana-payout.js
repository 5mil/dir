 ...
 *
 * Modes:
 *   dgb_integrated  — integrated HD wallet DGB address
 *   dgb_external    — any DGB D... address
 *   solana          — SOL base58 address
 *   custom          — any coin (ETH validated via eth-validator)
 *
...
function setDGBExternal(address) {
  if (!/^D[A-Za-z0-9]{33}$/.test(address))
    throw new Error('Invalid DGB address (must start with D, 34 chars).');
  return _write({ mode: 'dgb_external', address, coin: 'DGB' });
}

function setSolanaAddress(address) {
  if (!/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(address))
    throw new Error('Invalid Solana address (base58, 32–44 chars).');
...
// Legacy compat: getPayoutAddress / loadConfig
...
module.exports = {
  setDGBIntegrated, setDGBExternal, setSolanaAddress, setCustomAddress,
  readPayoutConfig, loadConfig, getPayoutAddress,
};
